...
# 中青
cp -f /Scripts/Task/Youth_Read.js /Scripts/Task/Youth_Read1.js
sed -i 's/YOUTH_READ/YOUTH_READ1/g' /Scripts/Task/Youth_Read1.js
sed -i '32,36s/^/\/\//g' /Scripts/Task/youth.js
sed -i "s/await readArticle();/\/\/await readArticle();/g" /Scripts/Task/youth.js
# 百度
sed -i "s/\&\&signs\[z].is_checkin!=0//g" /Scripts/Task/baidu_speed.js
cp -f /Scripts/Task/baidu_speed.js /Scripts/Task/baidu_speedtx.js
sed -i 's/||"true"/||"false"/g' /Scripts/Task/baidu_speedtx.js
sed -i "/await\ userInfo/i\\      if (\$.time(\"HH\") == \"06\") {\n        await withDraw(withcash)\n      }" /Scripts/Task/baidu_speedtx.js

npm install --loglevel error --prefix /Scripts/Task
